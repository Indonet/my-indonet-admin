...
function getCustLastMonthRange($custID, $fromyear = null, $frommonth = null, $toyear = null, $tomonth = null) {
    ...
    $sql = "SELECT * FROM " . TBL_CINVOICE . "
            ...
            AND transdate BETWEEN ? and ?
            ORDER BY transdate DESC";
    $rs = $axdb->execute($sql, array($custID, $frommonth, $to));
...
function getCustInfoAll($custID, $year, $month, $fromyear_array, $frommonth_array) { 
    global $axdb;
    $sql = "SELECT ACCOUNTNUM, NAME, KNOWNAS, SALESDISTRICTID, TYPETAGIHAN, MK_REGISTRATIONDATE, TYPECUST, DISTRICTNAME, INSTALATIONNAME, NPWPNAME, INVOICEADDRESS, INSTALATIONADDRESS, FAKTURPAJAKADDRESS, PHONE, EMAIL, FAX, MK_CUSTSTATUS FROM " . TBL_CINFO . " WHERE ACCOUNTNUM = ?";
    $rs = $axdb->execute($sql, array($custID));
    if ($rs) {
        $cinfo = $rs->GetArray();
        $cinfo['VIRTUAL_ACC'] = getCustBankAcc($custID);
        ...
        //$cinfo['ADDR_LIST'] = getInstallAddress($custID);        
	//$cinfo['INV_TOTAL'] = getCustTotalInvoice($custID, $year, $month); 
        for ($i=0; $i <= 2; $i++) { 
